...
bool ControllerB::update() {
	...
	int method = ConfigInt("Experiment");
	
	switch(method) {
		case 0:
			bestSample = updateWithSampler(0);
			break;
		case 1:
			bestSample = updateWithoutSampler(3);
			break;
		case 2:
			bestSample = updateWithoutSampler(4);
			break;
	}
	...
	stringstream str;
	if(bestSample != NULL)
		str<<method<<"\t"<<bestSample->parameters.transpose();
	else 
		str<<method<<"\t"<<VectorXf::Zero(6).transpose();
	traceString = str.str();
	doTrace();
